######################################################
#### Discretizing migratory behaviors			  ####
...
vi95 <- read.csv("volumeintersection.csv")
vi50 <- read.csv("volumeintersection50.csv")
look <- read.csv("../MethodComparison/migstatus-prelimlook.csv") %>%
...
length(which(mig$Look == "Intermediate"))
